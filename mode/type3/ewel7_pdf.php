...
if (in_array("35", $pdfline)) {
	...
	if ($objw->rs_sougolv <= 2) {
		$contents = preg_replace("/##RSSOGOBACK##/", "red", $contents);
	} else {
		$contents = preg_replace("/##RSSOGOBACK##/", "", $contents);
	}
	...
	if ($objw->rs_yomitorilv <= 2) {
		$contents = preg_replace("/##RSYOMITORIBACK##/", "red", $contents);
	} else {
		$contents = preg_replace("/##RSYOMITORIBACK##/", "", $contents);
	}
	...
	if ($objw->rs_rikailv <= 2) {
		$contents = preg_replace("/##RSRIKAIBACK##/", "red", $contents);
	} else {
		$contents = preg_replace("/##RSRIKAIBACK##/", "", $contents);
	}
	...
	if ($objw->rs_sentakulv <= 2) {
		$contents = preg_replace("/##RSSELECTBACK##/", "red", $contents);
	} else {
		$contents = preg_replace("/##RSSELECTBACK##/", "", $contents);
	}
	...
	if ($objw->rs_kirikaelv <= 2) {
		$contents = preg_replace("/##RSCHANGEBACK##/", "red", $contents);
	} else {
		$contents = preg_replace("/##RSCHANGEBACK##/", "", $contents);
	}


	$contents = preg_replace("/##RSINFOSC##/", round($objw->rs_jyoho, 1), $contents);
	$contents = preg_replace("/##RSINFOLV##/", $objw->rs_jyoholv, $contents);
	if ($objw->rs_jyoholv <= 2) {
		$contents = preg_replace("/##RSINFOBACK##/", "red", $contents);
	} else {
		$contents = preg_replace("/##RSINFOBACK##/", "", $contents);
	}
	...
	if ($objw->basiclv <= 2) {
		$contents = preg_replace("/##RSBACK##/", "red", $contents);
	} else {
		$contents = preg_replace("/##RSBACK##/", "", $contents);
	}

	$contents = preg_replace("/##RSEMOTIONLV##/", $objw->emotionlv, $contents);
	if ($objw->emotionlv <= 2) {
		$contents = preg_replace("/##RSEMOTIONBACK##/", "red", $contents);
	} else {
		$contents = preg_replace("/##RSEMOTIONBACK##/", "", $contents);
	}

	$contents = preg_replace("/##RSIMAGELV##/", $objw->imagelv, $contents);
	if ($objw->imagelv <= 2) {
		$contents = preg_replace("/##RSIMAGEBACK##/", "red", $contents);
	} else {
		$contents = preg_replace("/##RSIMAGEBACK##/", "", $contents);
	}
...
if (
	...
	$types = array(1, 2, 12, 59, 72, 82, 91);
	...
	if ($five && $_REQUEST['code'] != "PDF") {
		...
		if (in_array("12", $masterType) || in_array("72", $masterType) || in_array("82", $masterType) || in_array("91", $masterType)) {
			...
			} else if (in_array("91", $masterType)) {
				$wpaper = $obj->getAnswerPaper($wtwhere, 91);
			} else {
...
if (in_array("30", $pdfline)) {
	...
	//データ配列
	$array_dev = array(
		1 => "12.3899988842875", 2 => "10.8091355795122", 3 => "-11.3400630508907", 4 => "-11.1409935987687", 5 => "-11.4670977166782", 6 => "-11.2616469638557", 7 => "-12.9346044211069", 8 => "-10.9954272066392", 9 => "-11.3643939778109", 10 => "-13.6341834932924", 11 => "-10.904631776716", 12 => "-11.1985541645689", 13 => "-0.15956446241323", 14 => "0.183189766780266", 15 => "-0.115175657922674", 16 => "4605.50309924376"
	);
